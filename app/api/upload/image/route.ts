...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const path = join(cwd(), 'public', 'uploads', 'gallery', normalizeName);

        await writeFile(path, buffer);

        const imageUrl = `/uploads/gallery/${normalizeName}`.toString();
        ...
        return new Response(JSON.stringify({ path: imageUrl, image, success: "Image uploaded successfully" }), {
            status: 200,
        });
    } catch (error: unknown) {
        console.error(error);
