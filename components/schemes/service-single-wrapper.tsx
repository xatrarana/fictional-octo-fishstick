...
const ServiceDetailsWrapper: React.FC<ServiceDetailsProps> = async ({
  sid,
}: ServiceDetailsProps) => {
    ...
    <div className="space-y-6">
      <Breadcrumb className="p-1">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink href={`/schemes/${data.service.category.id}`}>
              {data.service.category.name}
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>{data.service.name}</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      <div className="">
              ...
              <div className="mt-4">
                {data.service.text && (
                  <DangerouslySetInnerHTML
                    content={dompurify.sanitize(data.service.text || "")}
                  />
                )}
              </div>
            </div>
